...
def plot_sa_vs_affinity(sa_score_csv, vina_csv):
    ...
    # --- Pareto frontier (minimize all four metrics: SA_score, affinity_kcal/mol, SCScore, NP_score) ---
    # A point is Pareto optimal if no other point is strictly better in all four metrics
    pareto_front = []
    data = merged[['SA_score', 'affinity_kcal/mol', 'SCScore', 'NP_score']].values
    for i, point in enumerate(data):
        # Check if any other point is strictly better in all metrics
        is_pareto = True
        for j, other in enumerate(data):
            if j != i and np.all(other < point):
                is_pareto = False
                break
        if is_pareto:
            pareto_front.append((point[0], point[1]))
    if pareto_front:
        ...
        # handles.append(Line2D([0], [0], color='red', linewidth=2, marker='o', markersize=6, label='Pareto frontier'))
        labels.append('Pareto frontier')
        ax.legend(handles=legend_elements, loc='upper left', title='Lipinski rules')  # labels=labels,
